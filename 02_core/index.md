...
```{file="select_star.sql"}
select * from little_penguins;
```
```{file="select_star.out"}
...
```{file="admin_commands.sql"}
.headers on
.mode markdown
select * from little_penguins;
```
```{file="admin_commands.out"}
...
```{file="specify_columns.penguins.sql"}
select
    species,
    island,
    sex
...
```
```{file="specify_columns.penguins.out"}
|  species  |  island   |  sex   |
|-----------|-----------|--------|
| Gentoo    | Biscoe    | MALE   |
| Adelie    | Dream     | FEMALE |
...
```{file="calculations.penguins.sql"}
select
...
limit 3;
```
```{file="calculations.penguins.out"}
...
```{file="rename_columns.penguins.sql"}
select
...
limit 3;
```
```{file="rename_columns.penguins.out"}
...
```{file="show_missing_values.penguins.sql"}
select
    flipper_length_mm / 10.0 as flipper_cm,
    body_mass_g / 1000.0 as weight_kg,
    island as where_found
...
limit 5;
```
```{file="show_missing_values.penguins.out"}
| flipper_cm | weight_kg | where_found |
|------------|-----------|-------------|
| 18.1       | 3.75      | Torgersen   |
| 18.6       | 3.8       | Torgersen   |
...
```{file="null_equality.penguins.sql"}
select distinct
    species,
    sex,
    island
...
```
```{file="null_equality.penguins.out"}
| species |  sex   | island |
|---------|--------|--------|
| Adelie  | FEMALE | Biscoe |
| Gentoo  | FEMALE | Biscoe |
...
```{file="null_inequality.penguins.sql"}
select distinct
    species,
    sex,
    island
...
where island = 'Biscoe' and sex != 'FEMALE';
```
```{file="null_inequality.penguins.out"}
| species | sex  | island |
|---------|------|--------|
| Adelie  | MALE | Biscoe |
| Gentoo  | MALE | Biscoe |
...
```{file="ternary_logic.penguins.sql"}
select null = null;
```
```{file="ternary_logic.penguins.out"}
| null = null |
|-------------|
|             |
```
...
```{file="safe_null_equality.penguins.sql"}
select
    species,
    sex,
    island
...
where sex is null;
```
```{file="safe_null_equality.penguins.out"}
| species | sex |  island   |
|---------|-----|-----------|
| Adelie  |     | Torgersen |
| Adelie  |     | Torgersen |
...
```{file="simple_sum.penguins.sql"}
select sum(body_mass_g) as total_mass
from penguins;
```
```{file="simple_sum.penguins.out"}
| total_mass |
|------------|
| 1437000.0  |
```
...
```{file="common_aggregations.penguins.sql"}
select
...
from penguins;
```
```{file="common_aggregations.penguins.out"}
...
```{file="count_behavior.penguins.sql"}
select
    count(*) as count_star,
...
from penguins;
```
```{file="count_behavior.penguins.out"}
...
```{file="simple_group.penguins.sql"}
select avg(body_mass_g) as average_mass_g
from penguins
group by sex;
```
```{file="simple_group.penguins.out"}
|  average_mass_g  |
|------------------|
| 4005.55555555556 |
| 3862.27272727273 |
...
```{file="unaggregated_columns.penguins.sql"}
select
    sex,
...
group by sex;
```
```{file="unaggregated_columns.penguins.out"}
|  sex   |  average_mass_g  |
|--------|------------------|
|        | 4005.55555555556 |
...
```{file="arbitrary_in_aggregation.penguins.sql"}
select
    sex,
    body_mass_g                   
from penguins
group by sex;
```
```{file="arbitrary_in_aggregation.penguins.out"}
|  sex   | body_mass_g |
|--------|-------------|
|        |             |
| FEMALE | 3800.0      |
...
```{file="filter_aggregation.penguins.sql"}
select
    sex,
    avg(body_mass_g) as average_mass_g
from penguins
...
```
```{file="filter_aggregation.penguins.out"}
| sex  |  average_mass_g  |
|------|------------------|
|      | 4005.55555555556 |
| MALE | 4545.68452380952 |
...
```{file="readable_aggregation.penguins.sql"}
select
    sex,
...
having average_mass_g > 4000.0;
```
```{file="readable_aggregation.penguins.out"}
| sex  | average_mass_g |
|------|----------------|
|      | 4005.6         |
| MALE | 4545.7         |
...
```{file="filter_aggregate_inputs.penguins.sql"}
select
    sex,
    round(
...
group by sex;
```
```{file="filter_aggregate_inputs.penguins.out"}
|  sex   | average_mass_g |
|--------|----------------|
|        | 3362.5         |
| FEMALE | 3417.3         |
...
```{file="update_work_job.sql"}
update work
set person = 'tae'
where person = 'tay';
```
```{file="show_after_update.memory.out"}
| person |    job    |
|--------|-----------|
| mik    | calibrate |
| mik    | clean     |
...
```{file="delete_rows.memory.sql:keep"}
delete from work
where person = 'tae';

select * from work;
```
```{file="delete_rows.memory.out"}
| person |    job    |
|--------|-----------|
| mik    | calibrate |
| mik    | clean     |
...
```{file="backing_up.memory.sql:keep"}
create table backup (
    person text not null,
    job text not null
);
...
```{file="cross_join.memory.sql:keep"}
select *
from work cross join job;
```
```{file="cross_join.memory.out"}
...
```{file="inner_join.memory.sql:keep"}
select *
from work inner join job
    on work.job = job.name;
```
```{file="inner_join.memory.out"}
| person |    job    |   name    | billable |
|--------|-----------|-----------|----------|
| mik    | calibrate | calibrate | 1.5      |
| mik    | clean     | clean     | 0.5      |
...
```{file="aggregate_join.memory.sql:keep"}
select
    work.person,
...
```
```{file="aggregate_join.memory.out"}
| person | pay |
|--------|-----|
| mik    | 2.0 |
| po     | 0.5 |
...
```{file="left_join.memory.sql:keep"}
select *
from work left join job
    on work.job = job.name;
```
```{file="left_join.memory.out"}
| person |    job    |   name    | billable |
|--------|-----------|-----------|----------|
| mik    | calibrate | calibrate | 1.5      |
| mik    | clean     | clean     | 0.5      |
...
```{file="aggregate_left_join.memory.sql:keep"}
select
    work.person,
    sum(job.billable) as pay
from work left join job
...
group by work.person;
```
```{file="aggregate_left_join.memory.out"}
| person | pay |
|--------|-----|
| mik    | 2.0 |
| po     | 0.5 |
...
```{file="full_outer_join.memory.sql"}
create table size (
    s text not null
);
...
```
```{file="full_outer_join.memory.out"}
|   s   | w |
|-------|---|
| light |   |
| heavy |   |
